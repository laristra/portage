...
class Simple_State_Wrapper {
 public:
   ...
   */
  explicit Simple_State_Wrapper(StateManager<Simple_Mesh_Wrapper> & manager) : manager_(manager) { }
...
/*  void mesh_get_data(Entity_kind on_what, std::string const& name,
                double **data) {
    auto it = manager_.find(name, on_what);
    if (it != manager_.end()) {
      (*data) = &(it->second[0]);
      return;
    }
...
/*  void mesh_add_data(Entity_kind on_what, std::string const& name,
                  double const **data) const {
    auto vec = manager_.add(name, on_what, *data);
  }
*/
  /*!
...
/*  const std::type_info& get_data_type(std::string const& var_name) const {
    auto it = manager_.find(var_name);
    if (it != manager_.end())
