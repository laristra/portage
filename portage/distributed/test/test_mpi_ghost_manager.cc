...
TEST(GhostManager, UpdateValues) {
  ...
  std::vector<std::vector<double>> owned[num_ranks]; /* values of owned cells that are ghost for some rank */
  std::vector<std::vector<double>> ghost[num_ranks]; /* values of ghost cells that are owned by some rank */
  std::vector<int> num_owned[num_ranks];             /* number of owned cells sent by each rank */
  std::vector<int> num_ghost[num_ranks];             /* number of ghost cells receive from each rank */
  std::vector<MPI_Request> requests;                 /* list of asynchronous MPI requests */
  ...
  if (rank > 0) {
    ...
    for (int i = 0; i < num_ranks; ++i) {
      if (i != rank) {
        int tag = num_ranks * (rank - 1) + i;
        MPI_Request request;
        MPI_Isend(owned[rank][i].data(), owned[rank][i].size(), MPI_DOUBLE, 0, tag, comm, &request);
        requests.emplace_back(request);
        MPI_Isend(ghost[rank][i].data(), ghost[rank][i].size(), MPI_DOUBLE, 0, tag + offset, comm, &request);
        requests.emplace_back(request);
      }
    }
  } else {
    ...
    for (int i = 1; i < num_ranks; ++i) {
      ...
      for (int j = 0; j < num_ranks; ++j) {
        if (i != j) {
          ...
          MPI_Request request;
          owned[i][j].resize(num_owned[i][j]);
          MPI_Irecv(owned[i][j].data(), num_owned[i][j], MPI_DOUBLE, i, tag, comm, &request);
          requests.emplace_back(request);
          ghost[i][j].resize(num_ghost[i][j]);
          MPI_Irecv(ghost[i][j].data(), num_ghost[i][j], MPI_DOUBLE, i, tag + offset, comm, &request);
          requests.emplace_back(request);
        }
      }
    }
  ...
  if (rank == 0) {
    for (int i = 0; i < num_ranks; ++i) {
      for (int j = 0; j < num_ranks; ++j) {
        ...
        ASSERT_EQ(num_sent_owned, num_recv_ghost);
        for (int k = 0; k < num_sent_owned; ++k) {
          ASSERT_DOUBLE_EQ(owned[i][j][k], ghost[j][i][k]);
        }
      }
    }
  }
