...
*/
  ...
  public:
    ...
    void distribute(SourceSwarm &source_swarm, SourceState &source_state,
      ...
      int targetNumPts = target_swarm.num_particles(Wonton::PARALLEL_OWNED);
      int sourceNumPts = source_swarm.num_particles(Wonton::PARALLEL_OWNED);
      ...
      ***************************************************************************/
      if (center == Meshfree::WeightCenter::Scatter) {
        ...
        for (int i = 0; i < src_info.new_num; ++i) {
          ...
          for (int k = 0; k < smoothing_length_sizes[sourceNumPts + i]; k++)
            for (int d = 0; d < smlen_dim; ++d)
