...
class PartDriver {
  ...
  */
        ...
        switch (onwhat) {
          case Entity_kind::CELL:
            internal_driver_[onwhat] =
                make_internal_driver<Entity_kind::CELL>(source_mesh_flat_,
                                                        source_state_flat_,
                                                        target_mesh_,
                                                        target_state_,
          ...
          case Entity_kind::NODE:
            internal_driver_[onwhat] =
                make_internal_driver<Entity_kind::NODE>(source_mesh_flat_,
                                                        source_state_flat_,
                                                        target_mesh_,
                                                        target_state_,
                                                        field_types,  executor);
...
#endif
    {
      for (Entity_kind onwhat : entity_kinds_) {
        switch (onwhat) {
          case Entity_kind::CELL:
            internal_driver_[onwhat] =
                make_internal_driver<Entity_kind::CELL>(source_mesh_,
                                                        source_state_,
                                                        target_mesh_,
                                                        target_state_,
                                                        field_types,  executor);
          ...
          case Entity_kind::NODE:
            internal_driver_[onwhat] =
                make_internal_driver<Entity_kind::NODE>(source_mesh_,
                                                        source_state_,
                                                        target_mesh_,
                                                        target_state_,
                                                        field_types,  executor);
  ...
  void
  interpolate_var(Entity_kind onwhat,
                  ...
                  T lower_bound, T upper_bound, double conservation_tol,
                  int max_fixup_iter) {
    if (source_redistributed_)
      boost::apply_visitor(interpolate_vistor<T, Flat_Mesh_Wrapper<>,
                           Flat_State_Wrapper<Flat_Mesh_Wrapper<>>>
                           (srcvarname, trgvarname, limiter,
                            partial_fixup_type, empty_fixup_type,
                            lower_bound, upper_bound,
                            conservation_tol, max_fixup_iter),
                           internal_driver_[onwhat]);
    else
      boost::apply_visitor(interpolate_vistor<T, SourceMesh, SourceState>
                           (srcvarname, trgvarname, limiter,
                            partial_fixup_type, empty_fixup_type,
                            lower_bound, upper_bound,
                            conservation_tol, max_fixup_iter),
 ...
 private:
  ...
  // A boost variant type to hold pointers to instances of the
  // internal driver with different mesh/state (Flat_mesh or
  // native_mesh) and different Entity_kind. When C++17
  // implementations mature and we switch to it we can use
  // std::variant
  typedef 
  boost::variant(std::unique_ptr<DriverInternal<Entity_kind::CELL,
                 SourceMesh,
                 SourceState>>,
                 std::unique_ptr<DriverInternal<Entity_kind::NODE,
                 SourceMesh,
                 SourceState>>,
                 std::unique_ptr<DriverInternal<Entity_kind::CELL,
                 Flat_Mesh_Wrapper<>,
                 Flat_State_Wrapper<Flat_Mesh_Wrapper<>>>,
                 std::unique_ptr<DriverInternal<Entity_kind::NODE,
                 Flat_Mesh_Wrapper<>,
  ...
  std::map<Entity_kind, std::unique_ptr<driver_t>> internal_driver_;


  template <typename T = double,
            Entity_kind ONWHAT, class SourceMesh2, class SourceState2>
  struct interpolate_visitor : public boost::static_visitor<> {
    interpolate_visitor(std::string srcvarname, std::string trgvarname,
                        Limiter_type limiter,
                        Partial_fixup_type partial_fixup_type,
                        Empty_fixup_type empty_fixup_type,
                        T lower_bound, T upper_bound,
                        double conservation_tol,
                        int max_fixup_iter) : srcvarname_(srcvarname),
                                              trgvarname_(trgvarname),
                                              limiter_(limiter),
                                              partial_fixup_type_(partial_fixup_type),
                                              empty_fixup_type_(empty_fixup_type),
                                              lower_bound_(lower_bound),
                                              upper_bound_(upper_bound),
                                              conservation_tol_(conservation_tol),
                                              max_fixup_iter_(max_fixup_iter)
    {}
    int operator()(std::unique_ptr<DriverInternal<ONWHAT, SourceMesh2, SourceState2>> driver_internal) {
      driver_internal->interpolate_var<T>(srcvarname_, trgvarname_,
                                          limiter_,
                                          partial_fixup_type_,
                                          empty_fixup_type_,
                                          lower_bound_, upper_bound_,
                                          conservation_tol_, max_fixup_iter_)
          }

   private:
    std::string srcvarname_, trgvarname_;
    Limiter_type limiter_;
    Partial_fixup_type partial_fixup_type_;
    Empty_fixup_type empty_fixup_type_;
    T lower_bound_, upper_bound_;
    double conservation_tol_;
    int max_fixup_iter_;
  }
  ...
  */
  ...
  std::unique_ptr<DriverInternal<ONWHAT, SourceMesh2, SourceState2>>
  make_internal_driver(SourceMesh2 const & source_mesh2,
    ...
    return
        std::make_unique<DriverInternal<ONWHAT, SourceMesh2, SourceState2>>
