...
TEST(search_direct_product, case1) {
  ...
  // Declare search
  Portage::SearchDirectProduct<Wonton::Direct_Product_Mesh_Wrapper,
  ...
  for (int i = 0; i < x_tgt.size() - 1; ++i) {
    for (int j = 0; j < y_tgt.size() - 1; ++j) {
      IntPoint<D> indices = {i,j};
      CellID id = tgt_wrapper.indices_to_cellid<D>(indices);
      std::vector<CellID> candidates;
      search(id, &candidates);
      n = 0;
      for (int i2 = i; i2 < i+2; ++i2) {
        for (int j2 = j; j2 < j+2; ++j2) {
          IntPoint<D> candidate = {i2,j2};
          CellID c_id = src_wrapper.indices_to_cellid<D>(candidate);
