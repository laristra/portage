...
TEST(TANGRAM_2D, test_matpoly_extract_points) {
  ...
  std::vector<Tangram::Point<2>> points = square_matpoly.points();
...
TEST(TANGRAM_2D, test_cellmatpoly_intersect) {
  ...
  for (int i = 0; i < nMatPoly; ++i) {
    ...
    Tangram::MatPoly<2> matPoly = cellmatpoly.get_ith_matpoly(i);

    // extract the matpoly points
    std::vector<Tangram::Point<2>> _matPoly_points = matPoly.points();

    // unfortunately we seem to need to use portage points only in intersection
    // so we need to convert from Tangram points to Portage points
    std::vector<Portage::Point<2>> matPoly_points;
    for (auto p : _matPoly_points)
      matPoly_points.push_back(Portage::Point<2>(p));
