...
std::vector<double> intersect_3Dpolys(
    const facetedpoly_t &srcpoly,
  ...
  std::vector<double> source_cell_bounds = {1e99,  -1e99, 1e99,
                                            -1e99, 1e99,  -1e99};
  ...
  for (int i = 0; i < num_verts; i++) cen += srcpoly.points[i];
  cen /= num_verts;
  ...
  for (auto const &target_cell_tet : target_tet_coords) {
    ...
    if (check_bb) {
      ...
      for (int j = 0; j < 3 && !disjoint; ++j)
        disjoint =
            (target_tet_bounds[2 * j] > source_cell_bounds[2 * j + 1] + bbeps ||
    ...
    // Accumulate moments:
    for (int i = 0; i < 4; i++) moments[i] += om[i];
  }

  delete[] verts;
  delete[] face_num_verts;
  for (int i = 0; i < num_faces; i++) delete[] face_vert_ids[i];
...
class IntersectR3D {
 public:
  ...
  /*! \brief Intersect a control volume of a target_entity with control volumes
   * of a set of source entities \param[in] tgt_entity Entity of target mesh to
   * intersect \param[in] src_entities Entity of source cells to intersect
   * against \return vector of Weights_t structure containing moments of
   * intersection
   */

  std::vector<Weights_t> operator()(const int tgt_entity,
                                    const std::vector<int> src_entities) const {
 ...
 public:
  ...
  std::vector<Weights_t> operator()(const int tgt_cell,
    ...
    for (int i = 0; i < nsrc; i++) {
      ...
      if (this_wt.weights[0] > 0.0) ninserted++;
    }
...
class IntersectR3D<NODE, SourceMeshType, TargetMeshType> {
 public:
   ...
   * \param[in] src_nodes  List of source nodes whose control volumes we will
   * intersect against \return vector of Weights_t structures containing moments
   * of intersection
   */
