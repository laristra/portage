    ...
    long icut)
{
  ...
  while ( r-l > 1 ) {
    ...
    i++; 
    while ( arr[prm[i]][icut] < arr[ia][icut] ) {
      i++;
    }

    j--;
    while ( arr[prm[j]][icut] > arr[ia][icut] ) {
      j--;
    }
...
// Return a list of BBox id's containing the query point
  template<long D>
void LocatePoint(const Point<D>& qp, const KDTree<D>* kdtree, 
    std::vector<long>& pfound)    
{
...
// given BBox (box)
  template<long D>
void Intersect(const IsotheticBBox<D>& box, const KDTree<D>* kdtree, 
    std::vector<long>& pfound)    
{
