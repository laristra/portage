...
template<int dim> void run(std::shared_ptr<Jali::Mesh> sourceMesh,
  ...
  // Wrappers for accessing source mesh data
  ...
  */  
  ...
  // Native jali state managers for source
  std::shared_ptr<Jali::State> sourceState(Jali::State::create(sourceMesh));

  // Portage wrappers for source state manager
  ...
  // In lieu of reaching into the MMDriver and timing the interface
  // reconstruction there, it is being timed here. Also, enclose it a block
  // so that it gets deleted before the main remapper is called. Otherwise we
  // will be storing material polygons twice
  ...
  }  // end source interface reconstruction (only needed for field initialization)
  ...
  // Wrappers for accessing target mesh data (done separately to keep
  // down the overall memory usage during source initialization)
  Wonton::Jali_Mesh_Wrapper targetMeshWrapper(*targetMesh);

  // Native jali state managers for target
  std::shared_ptr<Jali::State> targetState(Jali::State::create(targetMesh));

  // Portage wrappers for target state manager
