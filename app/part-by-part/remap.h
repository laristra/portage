...
void remap(std::string const& field, int nb_parts,
  ...
  using PartPair = Portage::PartPair<dim, Wonton::Jali_Mesh_Wrapper,
                                     Wonton::Jali_State_Wrapper>;
  ...
  auto candidates = remapper.template search<Portage::SearchKDTree>();
  auto weights = remapper.template intersect_meshes<Portage::IntersectRnD>(candidates);
  ...
  for (auto&& part : parts_registry) {
    ...
    if (params.order == 2) {
      ...
      auto gradients = remapper.compute_source_gradient(field, params.limiter,
                                                        ...
                                                        &source_part);
      remapper.template interpolate_mesh_var<T,
